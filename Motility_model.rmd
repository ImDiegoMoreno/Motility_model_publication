...
# --- 1. Define formula (same structure as motility model) ---
bf_beta_prog_loph <- bf(
  Proportion_Progressive_Transformed ~ season_bin,
  phi ~ season_bin
)

# --- 2. Compute observed mean for centering the Intercept prior ---
...
logit_mean_prog_loph  # check the value (e.g. ~ -1.40)

# # --- 3. Priors (regularising, same structure as motility models) ---
# priors_prog_loph <- c(
#   # Mean (mu) part
...
# 
#   # Phi (precision) part on log scale
#   prior(normal(log(10), 1), class = "Intercept", dpar = "phi"),
#   prior(normal(0, 0.3), class = "b", dpar = "phi")
# )
# 
# # --- 4. Fit model ---
...
#   family  = Beta(link = "logit", link_phi = "log"),  # <- log link for phi
...
```{r}
#saveRDS(m_beta_prog_loph, file = "models/m_beta_prog_loph.rds")
m_beta_prog_loph <- readRDS("models/m_beta_prog_loph.rds")
```
...
```{r}
# For Lophelia progressive
pp_check(m_beta_prog_loph, ndraws = 100, type = "dens_overlay")
pp_check(m_beta_prog_loph, type = "stat_grouped", stat = "mean", group = "season_bin")
pp_check(m_beta_prog_loph, type = "stat_grouped", stat = "sd",   group = "season_bin")
```
...
```{r}
# --- 1. Prepare data for Desmo progressive ---
model_data_beta_prog_desmo <- db3_max %>%
  filter(Species == "Desmo", time_after_min_round < 15) %>%
  mutate(
    Proportion_Progressive = Percent_Progressive / 100,
    N_obs = n(),
    # Apply Smithson & Verkuilen (2006) transformation to keep inside (0,1)
    Proportion_Progressive_Transformed = 
      (Proportion_Progressive * (N_obs - 1) + 0.5) / N_obs
  )

summary(model_data_beta_prog_desmo$Proportion_Progressive_Transformed)  # quick check

# --- 2. Define the Beta regression model for Desmo progressive ---
bf_beta_prog_desmo <- bf(
  Proportion_Progressive_Transformed ~ season_bin,
  phi ~ season_bin
)

# --- 3. Compute observed mean (for intercept prior centring) ---
...
```{r}
#saveRDS(m_beta_prog_desmo, file = "models/m_beta_prog_desmo.rds")
m_beta_prog_desmo <- readRDS("models/m_beta_prog_desmo.rds")
```
...
```{r}
# For Desmo progressive
pp_check(m_beta_prog_desmo, ndraws = 100, type = "dens_overlay")
pp_check(m_beta_prog_desmo, type = "stat_grouped", stat = "mean", group = "season_bin")
pp_check(m_beta_prog_desmo, type = "stat_grouped", stat = "sd",   group = "season_bin")
```
...
# --- 3. Extract posterior draws for both species ---
draws_desmo_prog    <- extract_mu_draws(m_beta_prog_desmo, "Desmo")
draws_lophelia_prog <- extract_mu_draws(m_beta_prog_loph, "Lophelia")

# --- 4. Combine ---
combined_draws_prog <- bind_rows(draws_desmo_prog, draws_lophelia_prog)

# --- 5. Summaries for labels ---
...
# --- 7. Plot ---
ggplot(combined_draws_prog, aes(x = .value, y = season_bin)) +
  stat_halfeye(
    aes(fill = Species, slab_alpha = after_stat(level)),
    .width = c(0.66, 0.95),
    point_interval = "median_hdi",
...
```{r}
# ======================================================================
# Progressive Motile VAP models for Desmo & Lophelia
# Consistent with % models: season_bin effects on mean and shape
# ======================================================================
...
# ---------- 0) Data ----------
data_prog_vap <- db3_max %>%
  filter(Species %in% c("Desmo", "Lophelia"),
         !is.na(Avg_Prog_Motile_VAP),
         Avg_Prog_Motile_VAP > 49,            # domain-specific filter
...
dat_desmo_prog_vap    <- filter(data_prog_vap, Species == "Desmo")
dat_loph_prog_vap     <- filter(data_prog_vap, Species == "Lophelia")
...
# ---------- 2) Priors per species ----------
# --- Desmo ---
log_mean_desmo_prog_vap <- mean(log(dat_desmo_prog_vap$Avg_Prog_Motile_VAP), na.rm = TRUE)
log_mean_desmo_prog_vap   # check value
priors_desmo_prog_vap <- c(
  prior(normal(4.795604, 0.5), class = "Intercept"),
  prior(normal(0, 0.3), class = "b"),
  prior(normal(2, 1.0), class = "Intercept", dpar = "shape"),
  prior(normal(0, 0.3), class = "b", dpar = "shape")
)

# --- Lophelia ---
log_mean_loph_prog_vap <- mean(log(dat_loph_prog_vap$Avg_Prog_Motile_VAP), na.rm = TRUE)
log_mean_loph_prog_vap   # check value
priors_loph_prog_vap <- c(
  prior(normal(4.655517, 0.5), class = "Intercept"),
  prior(normal(0, 0.3), class = "b"),
  prior(normal(2, 1.0), class = "Intercept", dpar = "shape"),
  prior(normal(0, 0.3), class = "b", dpar = "shape")
)
...
# # ---------- 3) Fit models ----------
# brm_desmo_prog_vap <- brm(
#   bf_prog_vap, data = dat_desmo_prog_vap,
#   family  = Gamma(link = "log"),
#   prior   = priors_desmo_prog_vap,
#   chains  = 4, iter = 4000, warmup = 2000,
#   control = list(adapt_delta = 0.98, max_treedepth = 12),
#   seed    = 101
# )
# 
# brm_loph_prog_vap <- brm(
#   bf_prog_vap, data = dat_loph_prog_vap,
#   family  = Gamma(link = "log"),
#   prior   = priors_loph_prog_vap,
#   chains  = 4, iter = 4000, warmup = 2000,
#   control = list(adapt_delta = 0.98, max_treedepth = 12),
#   seed    = 102
# )

# ---------- 4) Save & reload models ----------
#saveRDS(brm_desmo_prog_vap, "models/brm_desmo_prog_vap.rds")
#saveRDS(brm_loph_prog_vap,  "models/brm_loph_prog_vap.rds")

# later reuse:
brm_desmo_prog_vap <- readRDS("models/brm_desmo_prog_vap.rds")
brm_loph_prog_vap  <- readRDS("models/brm_loph_prog_vap.rds")

# ---------- 5) Prior & posterior predictive checks ----------
pp_check(brm_desmo_prog_vap, ndraws = 100, type = "dens_overlay")
pp_check(brm_desmo_prog_vap, type = "stat_grouped", stat = "mean", group = "season_bin")
pp_check(brm_desmo_prog_vap, type = "stat_grouped", stat = "sd", group = "season_bin")

pp_check(brm_loph_prog_vap, ndraws = 100, type = "dens_overlay")
pp_check(brm_loph_prog_vap, type = "stat_grouped", stat = "mean", group = "season_bin")
pp_check(brm_loph_prog_vap, type = "stat_grouped", stat = "sd", group = "season_bin")

# ---------- 6) Extract posterior draws ----------
extract_mu_draws_prog_vap <- function(fit, species_name) {
  fit %>%
    add_linpred_draws(newdata = data.frame(season_bin = c("end","mid","start")),
                      re_formula = NA, transform = TRUE) %>%
    select(.draw, season_bin, .linpred) %>%
    rename(.value = .linpred) %>%
    mutate(Species = species_name)
}

draws_desmo_prog_vap    <- extract_mu_draws_prog_vap(brm_desmo_prog_vap, "Desmo")
draws_loph_prog_vap     <- extract_mu_draws_prog_vap(brm_loph_prog_vap, "Lophelia")

combined_draws_prog_vap <- bind_rows(draws_desmo_prog_vap, draws_loph_prog_vap)

combined_summary_prog_vap <- combined_draws_prog_vap %>%
  group_by(Species, season_bin) %>%
  summarise(
    median = median(.value),
    lower  = quantile(.value, 0.025),
...
ggplot(combined_draws_prog_vap, aes(x = .value, y = season_bin)) +
  stat_halfeye(
    aes(fill = Species, slab_alpha = after_stat(level)),
    .width = c(0.66, 0.95),
    point_interval = "median_hdi",
  ...
  geom_text(
    data = combined_summary_prog_vap,
    aes(x = lower, y = season_bin, label = sprintf("%.0f", lower)),
    inherit.aes = FALSE, hjust = 1.2, size = 3.5, color = "grey30"
  ) +
  geom_text(
    data = combined_summary_prog_vap,
    aes(x = upper, y = season_bin, label = sprintf("%.0f", upper)),
    inherit.aes = FALSE, hjust = -0.2, size = 3.5, color = "grey30"
  ) +
  geom_richtext(
    data = combined_summary_prog_vap,
    aes(x = median, y = season_bin,
        label = paste0("<b>", sprintf("%.0f", median), "</b>")),
    fill = NA, label.color = NA,
    inherit.aes = FALSE, size = 3.5, vjust = 1.1, color = "black"
  ) +
  labs(
    title = "Posterior Distribution of Progressive Motile VAP by Season",
    x = "Estimated Mean Progressive VAP (µm/s)",
    y = NULL, fill = NULL
  ) +
  theme_classic(base_size = 14) +
  theme(
...
# ======================================================================
# Motile VAP models for Desmo & Lophelia
# Consistent with % and Progressive VAP models: season_bin effects on mean and shape
# ======================================================================
...
# ---------- 0) Data ----------
data_mot_vap <- db3_max %>%
  filter(Species %in% c("Desmo", "Lophelia"),
         !is.na(Avg_Motile_VAP),
         Avg_Motile_VAP > 0,              # Gamma requires strictly > 0
         time_after_min_round < 15) %>%
  mutate(
    season_bin = factor(season_bin, levels = c("start","mid","end")),
    Species    = factor(Species)
...
dat_desmo_mot_vap    <- filter(data_mot_vap, Species == "Desmo")
dat_loph_mot_vap     <- filter(data_mot_vap, Species == "Lophelia")

# ---------- 1) Formula ----------
bf_mot_vap <- bf(
  Avg_Motile_VAP ~ season_bin,
  shape ~ season_bin
)

# ---------- 2) Priors per species ----------
# --- Desmo ---
log_mean_desmo_mot_vap <- mean(log(dat_desmo_mot_vap$Avg_Motile_VAP), na.rm = TRUE)
log_mean_desmo_mot_vap   # check value
priors_desmo_mot_vap <- c(
  prior(normal(3.713249, 0.5), class = "Intercept"),
  prior(normal(0, 0.3), class = "b"),
  prior(normal(2, 1.0), class = "Intercept", dpar = "shape"),
  prior(normal(0, 0.3), class = "b", dpar = "shape")
)
...
# --- Lophelia ---
log_mean_loph_mot_vap <- mean(log(dat_loph_mot_vap$Avg_Motile_VAP), na.rm = TRUE)
log_mean_loph_mot_vap   # check value
priors_loph_mot_vap <- c(
  prior(normal(3.815233, 0.5), class = "Intercept"),
  prior(normal(0, 0.3), class = "b"),
  prior(normal(2, 1.0), class = "Intercept", dpar = "shape"),
  prior(normal(0, 0.3), class = "b", dpar = "shape")
)
...
# # ---------- 3) Fit models ----------
# brm_desmo_mot_vap <- brm(
#   bf_mot_vap, data = dat_desmo_mot_vap,
#   family  = Gamma(link = "log"),
#   prior   = priors_desmo_mot_vap,
#   chains  = 4, iter = 4000, warmup = 2000,
#   control = list(adapt_delta = 0.98, max_treedepth = 12),
#   seed    = 201
# )
# 
# brm_loph_mot_vap <- brm(
#   bf_mot_vap, data = dat_loph_mot_vap,
#   family  = Gamma(link = "log"),
#   prior   = priors_loph_mot_vap,
#   chains  = 4, iter = 4000, warmup = 2000,
#   control = list(adapt_delta = 0.98, max_treedepth = 12),
#   seed    = 202
# )
...
# # ---------- 4) Save & reload models ----------
# saveRDS(brm_desmo_mot_vap, "models/brm_desmo_mot_vap.rds")
# saveRDS(brm_loph_mot_vap,  "models/brm_loph_mot_vap.rds")

# later reuse:
brm_desmo_mot_vap <- readRDS("models/brm_desmo_mot_vap.rds")
brm_loph_mot_vap  <- readRDS("models/brm_loph_mot_vap.rds")

# ---------- 5) Prior & posterior predictive checks ----------
pp_check(brm_desmo_mot_vap, ndraws = 100, type = "dens_overlay")
pp_check(brm_desmo_mot_vap, type = "stat_grouped", stat = "mean", group = "season_bin")
pp_check(brm_desmo_mot_vap, type = "stat_grouped", stat = "sd", group = "season_bin")

pp_check(brm_loph_mot_vap, ndraws = 100, type = "dens_overlay")
pp_check(brm_loph_mot_vap, type = "stat_grouped", stat = "mean", group = "season_bin")
pp_check(brm_loph_mot_vap, type = "stat_grouped", stat = "sd", group = "season_bin")

# ---------- 6) Extract posterior draws ----------
extract_mu_draws_mot_vap <- function(fit, species_name) {
  fit %>%
    add_linpred_draws(newdata = data.frame(season_bin = c("end","mid","start")),
                      re_formula = NA, transform = TRUE) %>%
    select(.draw, season_bin, .linpred) %>%
...
draws_desmo_mot_vap    <- extract_mu_draws_mot_vap(brm_desmo_mot_vap, "Desmo")
draws_loph_mot_vap     <- extract_mu_draws_mot_vap(brm_loph_mot_vap, "Lophelia")

combined_draws_mot_vap <- bind_rows(draws_desmo_mot_vap, draws_loph_mot_vap)

combined_summary_mot_vap <- combined_draws_mot_vap %>%
  group_by(Species, season_bin) %>%
  summarise(
    median = median(.value),
    lower  = quantile(.value, 0.025),
...
ggplot(combined_draws_mot_vap, aes(x = .value, y = season_bin)) +
  stat_halfeye(
    aes(fill = Species, slab_alpha = after_stat(level)),
    .width = c(0.66, 0.95),
    point_interval = "median_hdi",
  ...
  labs(
    title = "Posterior Distribution of Motile VAP by Season",
    x = "Estimated Mean Motile VAP (µm/s)",
    y = NULL, fill = NULL
  ) +
  theme_classic(base_size = 14) +
  theme(
...
# Extract posterior predictions for Desmo model
check_draws <- brm_desmo_mot_vap %>%
  add_linpred_draws(newdata = data.frame(season_bin = c("start","mid","end")),
                    re_formula = NA, transform = TRUE)

check_summary <- check_draws %>%
  group_by(season_bin) %>%
  summarise(
    mean = mean(.linpred),
    median = median(.linpred),
    lower = quantile(.linpred, 0.025),
    upper = quantile(.linpred, 0.975)
  )
...
table(dat_desmo_mot_vap$season_bin)
tapply(dat_desmo_mot_vap$Avg_Motile_VAP, dat_desmo_mot_vap$season_bin, summary)
```
...
```{r}
# Are both egg levels present for each species?
model_data_check <- db3_max %>%
  filter(Species %in% c("Desmo","Lophelia"),
         time_after_min_round < 15) %>%
  mutate(egg_present = factor(egg_present, levels = c("no","yes")))

cat("\nCounts by species x egg_present:\n")
print(with(model_data_check, table(Species, egg_present, useNA="ifany")))

cat("\nCounts by species x egg_present (unique pairs):\n")
tmp_pairs <- model_data_check %>%
  mutate(pair_id = paste(Species, date, male, time_after_min_round, sep = "_")) %>%
  distinct(Species, pair_id, egg_present)
print(with(tmp_pairs, table(Species, egg_present, useNA="ifany")))


```

```{r}
# Replace with your fitted objects
fits <- list(
  m_motile_desmo = m_motile_desmo,
  m_motile_loph  = m_motile_loph,
  m_prog_desmo   = m_prog_desmo,
  m_prog_loph    = m_prog_loph
)

for(nm in names(fits)){
  cat("\n=================", nm, "=================\n")
  if (is.null(fits[[nm]])) { cat("MODEL IS NULL\n"); next }
  print(summary(fits[[nm]])$fixed)   # show fixed effects; you should see an egg_present effect
  cat("levels(egg_present) inside model:\n")
  print(levels(fits[[nm]]$data$egg_present))
  cat("distinct egg_present counts in model data:\n")
  print(table(fits[[nm]]$data$egg_present, useNA="ifany"))
}
...
```{r}
# ======================================================================
# Effect of Egg Presence on % Motile and % Progressive sperm
# Difference = Egg present – Egg absent
# ======================================================================
...
# ---------- 0) Data prep ----------
prepare_data <- function(db, response_col) {
  db %>%
    filter(Species %in% c("Desmo", "Lophelia"),
           time_after_min_round < 15,
           !is.na(.data[[response_col]])) %>%
    mutate(
      Proportion = .data[[response_col]] / 100,
      N_obs = n(),
      Proportion_Transformed = (Proportion * (N_obs - 1) + 0.5) / N_obs,
      egg_present = factor(egg_present, levels = c("no","yes")),
...
model_data_motile <- prepare_data(db3_max, "Percent_Motile")
model_data_prog   <- prepare_data(db3_max, "Percent_Progressive")

# ---------- 1) Formula ----------
bf_generic <- bf(
  Proportion_Transformed ~ egg_present,
  phi ~ egg_present
)
...
fit_species <- function(data, formula, priors, seed) {
  if (nrow(data) < 5 || n_distinct(data$egg_present) < 2) {
    warning("Not enough data to fit model for ", unique(data$Species))
    return(NULL)
  }
  brm(
    formula,
...
# --- Motile ---
m_motile_desmo <- fit_species(filter(model_data_motile, Species == "D. dianthus"),
                              bf_generic, priors_generic, seed = 101)
m_motile_loph  <- fit_species(filter(model_data_motile, Species == "D. pertusum"),
                              bf_generic, priors_generic, seed = 102)

# --- Progressive ---
m_prog_desmo <- fit_species(filter(model_data_prog, Species == "D. dianthus"),
                            bf_generic, priors_generic, seed = 201)
m_prog_loph  <- fit_species(filter(model_data_prog, Species == "D. pertusum"),
                            bf_generic, priors_generic, seed = 202)

# ---------- 4) Extract posterior differences ----------
extract_diff <- function(fit, species_label) {
  if (is.null(fit)) return(NULL)
  lv <- levels(fit$data$egg_present)
  newd <- data.frame(egg_present = factor(c("no","yes"), levels = lv))
  ep <- posterior_epred(fit, newdata = newd, re_formula = NA)
  stopifnot(ncol(ep) == 2)
  diff <- (ep[,2] - ep[,1]) * 100
  data.frame(Species = species_label, diff = diff)
}

# Motile
diffs_motile <- bind_rows(
  extract_diff(m_motile_desmo, "D. dianthus"),
  extract_diff(m_motile_loph,  "D. pertusum")
)

# Progressive
diffs_prog <- bind_rows(
  extract_diff(m_prog_desmo, "D. dianthus"),
  extract_diff(m_prog_loph,  "D. pertusum")
)

# ---------- 5) Summarise draws ----------
summarise_diffs <- function(diffs) {
  diffs %>%
    group_by(Species) %>%
    summarise(
      median = median(diff),
...
summary_motile <- summarise_diffs(diffs_motile)
summary_prog   <- summarise_diffs(diffs_prog)

# ---------- 6) Fancy plotting function ----------
species_cols <- c("D. dianthus" = "#9467bd", "D. pertusum" = "#f3c300")

plot_diffs_fancy <- function(diffs, summary, title, xlab) {
  ggplot(diffs, aes(x = diff, y = Species)) +
    stat_halfeye(
      aes(fill = Species, slab_alpha = after_stat(level)),
      .width = c(0.66, 0.95),
      point_interval = "median_hdi",
    ...
    geom_text(
      data = summary,
      aes(x = lower, y = Species, label = sprintf("%.1f", lower)),
      inherit.aes = FALSE, hjust = 1.2, size = 3.5, color = "grey30"
    ) +
    geom_text(
      data = summary,
      aes(x = upper, y = Species, label = sprintf("%.1f", upper)),
      inherit.aes = FALSE, hjust = -0.2, size = 3.5, color = "grey30"
    ) +
    geom_richtext(
      data = summary,
      aes(x = median, y = Species,
          label = paste0("<b>", sprintf("%.1f", median), "</b>")),
      fill = NA, label.color = NA,
      inherit.aes = FALSE, size = 3.5, vjust = -1.0, color = "black"
    ) +
    labs(
      title = title,
      subtitle = "Difference = Egg present – Egg absent",
      x = xlab,
      y = "Species"
    ) +
    theme_classic(base_size = 14) +
...
# ---------- 7) Final plots ----------
plot_motile_fancy <- plot_diffs_fancy(
  diffs_motile, summary_motile,
  "Posterior Distribution of Difference in % Motile Sperm",
  "Difference in % Motile"
)

plot_prog_fancy <- plot_diffs_fancy(
  diffs_prog, summary_prog,
  "Posterior Distribution of Difference in % Progressive Sperm",
...
```{r}
# ======================================================================
# Effect of Egg Presence on Motile & Progressive VAP (µm/s)
# Difference = Egg present – Egg absent
# ======================================================================
...
# ---------- 0) Data prep ----------
prepare_data_vap <- function(db, response_col) {
  db %>%
    filter(Species %in% c("Desmo", "Lophelia"),
           time_after_min_round < 15,
           !is.na(.data[[response_col]]),
           .data[[response_col]] > 49) %>%   # keep your domain-specific filter
    mutate(
      egg_present = factor(egg_present, levels = c("no","yes")),
      Species = recode(Species,
                       "Desmo" = "D. dianthus",
...
model_data_mot_vap  <- prepare_data_vap(db3_max, "Avg_Motile_VAP")
model_data_prog_vap <- prepare_data_vap(db3_max, "Avg_Prog_Motile_VAP")

# ---------- 1) Formula ----------
bf_vap <- bf(
  response ~ egg_present,
  shape ~ egg_present
)

# ---------- 2) Priors ----------
priors_vap <- c(
  prior(normal(4.5, 0.5), class = "Intercept"),   # log scale mean velocity ~ exp(4.5) ≈ 90 µm/s
  prior(normal(0, 0.3), class = "b"),             # effect of egg_present
  prior(normal(2, 1.0), class = "Intercept", dpar = "shape"), # dispersion
  prior(normal(0, 0.3), class = "b", dpar = "shape")
)

# ---------- 3) Fit models per species ----------
fit_species_vap <- function(data, response_col, formula, priors, seed) {
  if (nrow(data) < 5 || n_distinct(data$egg_present) < 2) {
    warning("Not enough data to fit model for ", unique(data$Species))
    return(NULL)
  }
  data <- data %>% rename(response = !!response_col)
  brm(
    formula,
    data = data,
    family = Gamma(link = "log"),
...
# --- Motile VAP ---
m_mot_vap_desmo <- fit_species_vap(filter(model_data_mot_vap, Species == "D. dianthus"),
                                   "Avg_Motile_VAP", bf_vap, priors_vap, seed = 301)
m_mot_vap_loph  <- fit_species_vap(filter(model_data_mot_vap, Species == "D. pertusum"),
                                   "Avg_Motile_VAP", bf_vap, priors_vap, seed = 302)

# --- Progressive VAP ---
m_prog_vap_desmo <- fit_species_vap(filter(model_data_prog_vap, Species == "D. dianthus"),
                                    "Avg_Prog_Motile_VAP", bf_vap, priors_vap, seed = 401)
m_prog_vap_loph  <- fit_species_vap(filter(model_data_prog_vap, Species == "D. pertusum"),
                                    "Avg_Prog_Motile_VAP", bf_vap, priors_vap, seed = 402)

# ---------- 4) Extract posterior differences ----------
extract_diff_vap <- function(fit, species_label) {
  if (is.null(fit)) return(NULL)
  lv <- levels(fit$data$egg_present)
  newd <- data.frame(egg_present = factor(c("no","yes"), levels = lv))
  ep <- posterior_epred(fit, newdata = newd, re_formula = NA)
  stopifnot(ncol(ep) == 2)
  diff <- ep[,2] - ep[,1]  # difference in µm/s
  data.frame(Species = species_label, diff = diff)
}

# Motile VAP
diffs_mot_vap <- bind_rows(
  extract_diff_vap(m_mot_vap_desmo, "D. dianthus"),
  extract_diff_vap(m_mot_vap_loph,  "D. pertusum")
)

# Progressive VAP
diffs_prog_vap <- bind_rows(
  extract_diff_vap(m_prog_vap_desmo, "D. dianthus"),
  extract_diff_vap(m_prog_vap_loph,  "D. pertusum")
)
...
summary_mot_vap  <- summarise_diffs(diffs_mot_vap)
summary_prog_vap <- summarise_diffs(diffs_prog_vap)
...
# ---------- 7) Final plots ----------
plot_mot_vap_fancy <- plot_diffs_fancy(
  diffs_mot_vap, summary_mot_vap,
  "Posterior Distribution of Difference in Motile VAP",
  "Difference in Motile VAP (µm/s)"
)

plot_prog_vap_fancy <- plot_diffs_fancy(
  diffs_prog_vap, summary_prog_vap,
  "Posterior Distribution of Difference in Progressive VAP",
  "Difference in Progressive VAP (µm/s)"
)
...
```{r}
# --- Inspect the data that feeds the Motile VAP models ---
mot_vap_raw <- db3_max %>%
  filter(Species %in% c("Desmo","Lophelia"),
         time_after_min_round < 15,
         !is.na(Avg_Motile_VAP),
...
cat("\nCounts by Species (after filters):\n")
print(table(mot_vap_raw$Species))

cat("\nCounts by Species x egg_present (after filters):\n")
print(with(mot_vap_raw, table(Species, egg_present, useNA = "ifany")))

cat("\nDesmo summary of Avg_Motile_VAP (after filters):\n")
print(summary(subset(mot_vap_raw, Species == "Desmo")$Avg_Motile_VAP))
```
