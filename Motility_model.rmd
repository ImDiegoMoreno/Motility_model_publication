...
ggplot(db3_max_binned, aes(x = Species, y = time_bin, fill = Percent_Motile)) +
  geom_tile() +
...
Code based on the logit_mean
```{r}
# compute observed logit mean (you already did this)
obs_mean <- mean(model_data_beta$Proportion_Motile, na.rm = TRUE)
logit_mean <- qlogis(obs_mean)   # = -1.857484

# Recommended priors (regularising)
priors_recommended <- c(
  # mean (mu) part on logit scale
  prior(student_t(3, -1.8575, 1), class = "Intercept"),
  prior(normal(0, 0.75), class = "b"),

  # phi part: choose either a Gamma prior on phi-intercept...
  prior(gamma(2, 0.01), class = "Intercept", dpar = "phi"),
  prior(normal(0, 0.5), class = "b", dpar = "phi")

  # --- ALTERNATIVE: if you prefer priors on log(phi) instead:
  # prior(student_t(3, 3, 1.5), class = "Intercept", dpar = "phi"),
  # prior(normal(0, 0.5), class = "b", dpar = "phi")
)

# See what brms expects
get_prior(bf_beta_het, data = model_data_beta, family = Beta(link = "logit"))

# Prior predictive check (sample only from prior)
m_prior_only <- brm(
  formula = bf_beta_het,
...
pp_check(m_prior_only, type = "dens_overlay") + ggtitle("Prior predictive (only) — check plausibility")

# If that looks sensible, fit the full model
m_beta <- brm(
  formula = bf_beta_het,
  data = model_data_beta,
  family = Beta(link = "logit"),
...
# Desmo
emm_desmo_link <- emmeans(brm_ml_Desmo_beta, ~ season_bin, type = "link")
draws_desmo_pct <- emm_desmo_link %>%
  gather_emmeans_draws() %>%
  mutate(Species = "Desmo",
         .value = plogis(.value) * 100)

# Lophelia
emm_lo_link <- emmeans(m_beta, ~ season_bin, type = "link")
draws_lo_pct <- emm_lo_link %>%
  gather_emmeans_draws() %>%
  mutate(Species = "Lophelia",
         .value = plogis(.value) * 100)
...
combine
```{r}
combined_draws <- bind_rows(draws_desmo_pct, draws_lo_pct)

combined_summary <- combined_draws %>%
  group_by(Species, season_bin) %>%
  summarise(median_hdi(.value, .width = 0.95), .groups = "drop")
...
#   ) %>%
#   mutate(
#     Species_strip = factor(
#       Species,
#       levels = species_levels,
#       labels = species_strip
#     )
#   )
# 
# # --- Plot with half-eyes + numeric labels ---
# ggplot(combined_draws, aes(x = .value, y = season_bin, fill = Species)) +
#   ggdist::stat_halfeye(
#     .width = c(0.66, 0.95),
#     point_interval = "median_hdi",
#     aes(slab_alpha = after_stat(level)),
#     trim = TRUE
#   ) +
#   scale_fill_manual(
#     values = c(Desmo = "#9467bd", Lophelia = "#f3c300"),
...
#   ) +
#   # Add labels (bounds + median)
#   geom_text(
#     data = combined_summary,
#     aes(
#       x = median,
#       y = season_bin,
#       label = paste0(sprintf("%.1f", lower), "   ",
#                      sprintf("%.1f", median), "   ",
#                      sprintf("%.1f", upper))
#     ),
#     inherit.aes = FALSE,
...
#   ) +
#   labs(
#     title = "Posterior Distribution of Mean Percentage Motile",
...
# Prepare combined_summary for label positioning
combined_summary <- combined_draws %>%
  group_by(Species, season_bin) %>%
  summarise(
    median = median(.value),
...
ggplot(combined_draws, aes(x = .value, y = season_bin, fill = Species)) +
  ...
  # --- Add median label (center-aligned, bold)
